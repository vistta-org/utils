...
request.setAuthorization = (token) => {
  runtime.__internal_request ||= {};
  runtime.__internal_request.auth = token;
};
...
request.use = (hook) => {
  ...
  runtime.__internal_request ||= {};
  runtime.__internal_request.hooks ||= [];
  runtime.__internal_request.hooks.push(hook);
  return () => {
    runtime.__internal_request.hooks = (runtime.__internal_request?.hooks || []).filter((current) => current !== hook);
  };
};
...
const runtime = /** @type {RequestRuntime} */ (globalThis);
...
async function buildHeaders(url, headers, contentType, method) {
  ...
  if (runtime.__internal_request?.auth) {
    if (typeof runtime.__internal_request.auth === "function")
      headers.set("authorization", `Bearer ${await runtime.__internal_request.auth()}`);
    else headers.set("authorization", `Bearer ${runtime.__internal_request.auth}`);
  }
...
async function runRequestHooks(context) {
  const hooks = runtime.__internal_request?.hooks || [];
