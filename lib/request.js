 ...
 */
const request = (url, { method = "get", controller, headers, params, timeout, content, stream, ...options } = {}) => {
  ...
  const response = new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (!response.ok) return reject(new Error((await response.text()) || response.status + "" + response.statusText));
      if (stream)
        return resolve({
          data: response.body,
