 ...
 */
request.setAuthorization = (token) => (runtime.__v_req_auth = token);

export { request };

//

const runtime = /** @type {typeof globalThis & { __v_req_auth?: string | (() => string | Promise<string>) }} */ (globalThis);
...
async function buildHeaders(url, headers, contentType, method) {
  ...
  if (runtime.__v_req_auth) {
    if (typeof runtime.__v_req_auth === "function") headers.set("authorization", `Bearer ${await runtime.__v_req_auth()}`);
    else headers.set("authorization", `Bearer ${runtime.__v_req_auth}`);
  }
