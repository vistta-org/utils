 ...
 *
 * @param {string | () => string} token - The bearer token to set.
 */
request.setAuthorization = (token) => (window.__v_req_auth = token);
...
async function buildHeaders(url, headers, contentType, method) {
  ...
  if (window.__v_req_auth) {
    if (typeof window.__v_req_auth === "function") headers.set("authorization", `Bearer ${await window.__v_req_auth()}`);
    else headers.set("authorization", `Bearer ${window.__v_req_auth}`);
  }
