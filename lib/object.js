...
export function flatten(object, separator = ".", transformer) {
  const helper = (target, prefix) => {
    ...
    for (let i = 0, len = keys.length; i < len; i++) {
      ...
      if (typeof target[_key] === "object" && target[_key] !== null) flattened = { ...flattened, ...helper(target[_key], key) };
...
export function proxy(target, options = {}) {
  ...
  function helper(obj, path) {
    ...
    const handlers = {};
    if (options.apply)
      handlers.apply = (target, thisArg, args) => options.apply.call(target, path, thisArg, args, () => Reflect.apply(target, thisArg, args));
    if (options.construct)
      handlers.construct = (target, args, newTarget) =>
        options.construct.call(target, path, args, newTarget, () => Reflect.construct(target, args, newTarget));
    handlers.defineProperty = (target, property, descriptor) => {
      if (!options.defineProperty) return false;
      const fullPath = [...path, property];
      return options.defineProperty.call(target, fullPath, descriptor, () => Reflect.defineProperty(target, property, descriptor));
    };
    if (options.deleteProperty || options.readonly)
      handlers.deleteProperty = (target, property) => {
        if (options.readonly) return false;
        const fullPath = [...path, property];
        return options.deleteProperty.call(target, fullPath, () => Reflect.deleteProperty(target, property));
      };
    if (options.get)
    ...
    if (options.getOwnPropertyDescriptor)
      handlers.getOwnPropertyDescriptor = (target, property) =>
        options.getOwnPropertyDescriptor.call(target, [...path, property], () => Reflect.getOwnPropertyDescriptor(target, property));
    if (options.getPrototypeOf) handlers.getPrototypeOf = (target) => options.getPrototypeOf.call(target, path, () => Reflect.getPrototypeOf(target));
    if (options.has) handlers.has = (target, property) => options.has.call(target, [...path, property], () => Reflect.has(target, property));
    if (options.isExtensible) handlers.isExtensible = (target) => options.isExtensible.call(target, path, () => Reflect.isExtensible(target));
    if (options.ownKeys) handlers.ownKeys = (target) => options.ownKeys.call(target, path, () => Reflect.ownKeys(target));
    if (options.preventExtensions)
      handlers.preventExtensions = (target) => options.preventExtensions.call(target, path, () => Reflect.preventExtensions(target));
    if (options.set || options.readonly)
      handlers.set = (target, property, value, receiver) => {
        if (options.readonly) return false;
        const oldValue = target[property];
        const fullPath = [...path, property];
        const newValue = options.recursive !== false && typeof value === "object" && value !== null ? helper(value, fullPath) : value;
        return options.set.call(target, fullPath, newValue, oldValue, () => Reflect.set(target, property, newValue, receiver));
      };
    handlers.setPrototypeOf = (target, prototype) => {
      if (!options.setPrototypeOf) return false;
      return options.setPrototypeOf.call(target, path, prototype, () => Reflect.setPrototypeOf(target, prototype));
    };
