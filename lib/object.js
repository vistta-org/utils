  ...
  function helper(obj, path) {
    ...
    if (options.apply)
      handlers.apply = (target, thisArg, args) =>
        options.apply.call(target, path, thisArg, args, () => Reflect.apply(target, thisArg, args));
    ...
    if (options.getPrototypeOf)
      handlers.getPrototypeOf = (target) => options.getPrototypeOf.call(target, path, Reflect.getPrototypeOf(target));
    if (options.has)
      handlers.has = (target, property) => options.has.call(target, [...path, property], Reflect.has(target, property));
    if (options.isExtensible)
      handlers.isExtensible = (target) => options.isExtensible.call(target, path, Reflect.isExtensible(target));
    ...
    if (options.preventExtensions)
      handlers.preventExtensions = (target) =>
        options.preventExtensions.call(target, path, Reflect.preventExtensions(target));
    if (options.set || options.readonly)
      handlers.set = (target, property, value, receiver) => {
        ...
        const fullPath = [...path, property];
        const newValue =
          options.recursive !== false && typeof value === "object" && value !== null ? helper(value, fullPath) : value;
