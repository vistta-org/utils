  ...
  function helper(obj, path) {
    ...
    if (options.apply)
      handlers.apply = (target, thisArg, args) =>
        options.apply.call(target, path, thisArg, args, () => target.apply(thisArg, ...args));
    if (options.construct)
      handlers.construct = (target, args) => options.construct.call(target, path, args, () => new target(...args));
    handlers.defineProperty = (target, property, descriptor) => {
      if (!options.defineProperty) return true;
    ...
    if (options.getPrototypeOf)
      handlers.getPrototypeOf = (target) => options.getPrototypeOf.call(target, path, Object.getPrototypeOf(target));
    if (options.has) handlers.has = (target, property) => options.has.call(target, [...path, property], !!target[property]);
    if (options.isExtensible)
      handlers.isExtensible = (target) => options.isExtensible.call(target, path, Object.isExtensible(target));
    if (options.ownKeys) handlers.ownKeys = (target) => options.ownKeys.call(target, path, Object.keys(target));
    if (options.preventExtensions)
      handlers.preventExtensions = (target) =>
        options.preventExtensions.call(target, path, Object.preventExtensions(target));
    handlers.set = (target, property, newValue) => {
      if (options.readonly) return true;
      const oldValue = target[property];
      const fullPath = [...path, property];
      if (options.set)
        return options.set.call(target, fullPath, newValue, oldValue, () => ((target[property] = newValue), true));
      target[property] = newValue;
      return true;
    };
    handlers.setPrototypeOf = (target, prototype) => {
      if (!options.setPrototypeOf) return true;
      return options.setPrototypeOf.call(target, path, prototype, () => Object.setPrototypeOf(target, prototype));
    };
